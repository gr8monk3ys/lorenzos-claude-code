...
This is a Claude Code plugin repository providing **66 slash commands**, **25 specialized AI agents**, **23 auto-activating skills**, **4 multi-agent orchestrators**, **4 MCP servers** (minimal profile, CLI-first approach), **18 hooks**, and **3 context modes** for modern web development (Next.js 15, TypeScript, React, Vue, Angular, Svelte, Supabase). Current version: **2.2.0**.
...
- `.claude/skills/` - Auto-activating skill files (flat structure, 23 skills)
- `.claude/orchestrators/` - Multi-agent workflow orchestrators
- `.claude/hooks/` - Pre-configured automation hooks (18 total)
...
**Skills by category (23 total):**

- **Development**: `api-development`, `frontend-development`, `database-operations`, `devops-automation`, `python-refactoring`
- **Quality**: `code-quality`, `verification-first`, `spec-compliance`, `eval-harness`, `auto-test-prompt`, `circuit-breaker`
...
### Hooks System (18 pre-configured)
...
- **detect-duplication.sh** (PostToolUse) - Detects code duplication using jscpd
- **detect-deadcode.sh** (PostToolUse) - Detects unused exports using knip
- **test-gate.sh** (PreToolUse) - Blocks git commits until tests pass
- **circuit-breaker.sh** (PostToolUse) - Detects thrashing patterns and prevents stuck loops
...
2. Add to `commands` array in [plugin.json](.claude-plugin/plugin.json):
   ...
   {
     "name": "cmd-name",
     "path": ".claude/commands/category/cmd-name.md",
     "description": "..."
   }
   ```
...
2. Add to `orchestrators` array in [plugin.json](.claude-plugin/plugin.json):
   ...
   {
     "name": "workflow-name",
     "path": ".claude/orchestrators/workflow-name.md",
     "description": "..."
   }
   ```
...
### Commands by Category (66 total, 12 categories)
...
- **workflow/** (15): `/wizard`, `/fix-pr`, `/handoff`, `/resume`, `/ledger`, `/chain`, `/harness`, `/wiggum`, `/architect`, `/map`, `/plan-init`, `/checkpoint`, `/eval`, `/learn`, `/blast-radius`
